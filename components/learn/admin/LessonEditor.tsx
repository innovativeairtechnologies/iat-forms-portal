  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? 'bg-[#f0faf4] dark:bg-emerald-500/10 text-[#089447] dark:text-emerald-400'
          : 'text-gray-400 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 hover:text-gray-700 dark:hover:text-zinc-200',
      )}
    >
      {children}
    </button>
...
export default function LessonEditor({
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-5 flex items-center justify-between">
        ...
        {viewHref && (
          <Link
            href={viewHref}
            target="_blank"
            className="inline-flex items-center gap-1.5 text-[12.5px] font-medium text-gray-400 dark:text-zinc-500 transition-colors hover:text-[#089447] dark:hover:text-emerald-400"
          >
      ...
      <p className="mb-1 text-[12px] font-semibold uppercase tracking-wide text-gray-400 dark:text-zinc-500">{moduleTitle}</p>
      <input
        value={title}
        onChange={e => setTitle(e.target.value)}
        placeholder="Lesson title"
        className="mb-4 w-full border-0 bg-transparent text-[26px] font-bold tracking-tight text-[#0a0a0b] dark:text-white outline-none placeholder:text-gray-300 dark:placeholder:text-zinc-500"
      />

      <div className="mb-4 flex flex-wrap items-center gap-4 rounded-xl border border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 px-4 py-3 shadow-card-sm dark:shadow-none">
        <label className="flex items-center gap-2 text-[13px] text-gray-600 dark:text-zinc-300">
          <span className="font-medium">Est. minutes</span>
          <input
            type="number" min={1} value={minutes}
            onChange={e => setMinutes(Number(e.target.value))}
            className="w-16 rounded-lg border border-gray-200 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200 px-2 py-1 text-[13px] outline-none focus:border-[#089447] dark:focus:border-emerald-400"
          />
        </label>
        <button
          type="button"
          ...
          className={cn(
            ...
            published ? 'bg-[#f0faf4] dark:bg-emerald-500/10 text-[#077a3c] dark:text-emerald-400' : 'bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-zinc-400',
          )}
        >
          <span className={cn('h-2 w-2 rounded-full', published ? 'bg-[#089447] dark:bg-emerald-400' : 'bg-gray-400 dark:bg-zinc-500')} />
        ...
        <div className="ml-auto">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl bg-[#089447] dark:bg-emerald-500 px-4 py-2 text-[13px] font-semibold text-white transition-all hover:bg-[#077a3c] dark:hover:bg-emerald-400 disabled:opacity-60"
          >
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 shadow-card dark:shadow-none">
        <div className="flex flex-wrap items-center gap-0.5 border-b border-gray-100 dark:border-zinc-800 bg-gray-50/60 dark:bg-zinc-900 px-2 py-1.5">
        ...
        <div className="px-6 py-5 dark:bg-zinc-900">
