...
export default function EmployeeShell({ employee, children }: { employee: Employee; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-[#F7F6F3] dark:bg-gray-950">
      ...
      {/* ── Desktop sidebar ── */}
      <aside className="hidden md:flex w-[220px] bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 flex-col flex-shrink-0 h-screen sticky top-0">
        ...
        <div className="p-2 border-t border-gray-100 dark:border-gray-800 space-y-0.5">
          <div className="flex items-center gap-2.5 px-3 py-2.5">
            <div className="w-7 h-7 rounded-full bg-gray-800 dark:bg-gray-700 flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0">
              {initials}
            </div>
      ...
      {open && (
        <div className="md:hidden fixed inset-0 z-50 flex" onClick={() => setOpen(false)}>
          ...
          <div className="relative w-[260px] bg-white dark:bg-gray-900 flex flex-col h-full shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-3 border-t border-gray-100 dark:border-gray-800 space-y-0.5">
              <button onClick={logout}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all">
                <LogOut size={15} className="text-gray-300 dark:text-gray-600" />
                Sign Out
              </button>
            </div>
