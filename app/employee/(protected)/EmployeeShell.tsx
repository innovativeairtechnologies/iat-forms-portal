...
export default function EmployeeShell({ employee, children }: { employee: Employee; children: React.ReactNode }) {
    ...
    <>
      {NAV.map(({ href, label, icon: Icon }) => {
        ...
        return (
          ...
          >
            <Icon size={15} className={active ? 'text-[#089447]' : 'text-gray-400 dark:text-gray-500'} />
            {label}
          </Link>
        )
      })}
  ...
  return (
    <div className="min-h-screen flex bg-[#F7F6F3] dark:bg-zinc-950">
      ...
      {/* ── Desktop sidebar ── */}
      <aside className="hidden md:flex w-[220px] bg-white dark:bg-zinc-900 border-r border-gray-100 dark:border-zinc-800 flex-col flex-shrink-0 h-screen sticky top-0">
        <div className="flex items-center gap-2.5 px-4 pt-5 pb-5">
          <Link href="/employee/profile" className="flex items-center gap-2.5 group flex-1 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-bold text-gray-900 dark:text-white leading-none tracking-tight group-hover:text-[#089447] transition-colors">IAT Portal</p>
        ...
        <div className="p-2 border-t border-gray-100 dark:border-zinc-800 space-y-0.5">
          <div className="flex items-center gap-2.5 px-3 py-2.5">
            <div className="w-7 h-7 rounded-full bg-gray-800 dark:bg-zinc-700 flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0">
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-semibold text-gray-700 dark:text-gray-200 leading-none truncate">{employee.name || employee.email}</p>
              <p className="text-[11px] text-gray-400 mt-0.5 truncate">{employee.job_title || 'Employee'}</p>
            </div>
            <ThemeToggle />
          </div>
          <button onClick={logout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-zinc-800 hover:text-gray-700 dark:hover:text-gray-300 transition-all">
      ...
      {/* ── Mobile top bar ── */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 bg-white dark:bg-zinc-900 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between px-4 h-14">
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <button onClick={() => setOpen(true)} className="text-gray-600 dark:text-gray-400 p-1"><Menu size={20} /></button>
        </div>
      </div>
        ...
        <div className="md:hidden fixed inset-0 z-50 flex" onClick={() => setOpen(false)}>
          ...
          <div className="relative w-[260px] bg-white dark:bg-zinc-900 flex flex-col h-full shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 dark:border-zinc-800">
              <span className="text-[13px] font-bold text-gray-900 dark:text-white">IAT Portal</span>
              <button onClick={() => setOpen(false)} className="text-gray-400 dark:text-gray-500 p-1"><X size={18} /></button>
            </div>
            <nav className="flex-1 px-3 py-3 space-y-0.5">
              <NavLinks />
            </nav>
            <div className="p-3 border-t border-gray-100 dark:border-zinc-800 space-y-0.5">
              <button onClick={logout}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-zinc-800 transition-all">
                <LogOut size={15} className="text-gray-300 dark:text-gray-600" />
                Sign Out
              </button>
            </div>
