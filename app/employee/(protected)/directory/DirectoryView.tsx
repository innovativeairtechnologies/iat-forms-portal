...
/* The employee-facing directory. Chart view = the read-only org chart (canEdit
   ...
   quick name/contact lookups. Admins edit the hierarchy at /admin/org-chart. */

export default function DirectoryView({ employees }: { employees: OrgEmployee[] }) {
  const [view, setView] = useState<'chart' | 'list'>('chart')
  const [query, setQuery] = useState('')
  const [dept, setDept] = useState<string | null>(null)

  // Employees only ever see people who are visible on the chart.
  const visible = useMemo(() => employees.filter((e) => e.org_visible), [employees])
  ...
  if (view === 'chart') {
    return <OrgChart employees={employees} canEdit={false} title="Directory" toolbarExtra={toggle} />
  }
    ...
    <div className="flex-1 min-h-0 flex flex-col overflow-hidden bg-zinc-50 dark:bg-[#0a0a0b]">
      <div className="flex items-center gap-3 px-5 h-14 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h1 className="text-[15px] font-bold text-zinc-900 dark:text-white">Directory</h1>
