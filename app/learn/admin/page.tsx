...
export default async function LearnAdminPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <Link
        href="/learn"
        className="mb-5 inline-flex items-center gap-1.5 text-[13px] font-medium text-gray-500 transition-colors hover:text-gray-900"
      >
        <ArrowLeft size={15} /> Back to Learn
      </Link>

      <header className="mb-7">
        <h1 className="text-[26px] font-bold tracking-tight text-[#0a0a0b]">Learn Admin</h1>
        ...
        <div className="mt-4 flex items-center gap-5 text-[13px]">
          <span className="flex items-center gap-1.5 text-gray-600">
            <Layers size={14} className="text-[#089447]" /> {categories.length} categories
          </span>
          <span className="flex items-center gap-1.5 text-gray-600">
            <BookOpen size={14} className="text-[#089447]" /> {modules.length} subjects · {lessons.length} lessons
          </span>
          {pendingCount > 0 && (
