...
| **Calendar** | What's due / scheduled? | Month grid of `deal_follow_ups` (date-fns). Overdue = rose, due-today = brand green, scheduled = sky, done = muted. Click a day → detail list with mark-done / open-deal / delete. **New event** (2026-07-21, migration 064): a "New event" button + per-day "Add event" composer (date, text, optional deal link) creates a `deal_follow_ups` row. A **standalone event** (`deal_id` NULL) isn't tied to any deal, so — unlike a deal follow-up — it **survives DryWare syncs** (deal follow-ups cascade-delete when their project is pruned). |
